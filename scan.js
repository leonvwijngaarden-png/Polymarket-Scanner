...
async function main() {
  ...
  for (const city of dueCities) {
    ...
    try {
      ...
      /* ── Bracket recommendation using backtest bias ──
         Start with the exact forecast bucket, then shift by the city's
         historical bias (e.g. if forecast runs warm +1 bucket on average,
         shift recommendation down 1 bucket toward the actual resolution). */
      ...
      if (bt && exactIdx != null && bt.biasShift !== 0) {
        /* biasShift > 0 means forecast runs warm → shift recommendation down */
        const shiftedIdx = Math.max(0, Math.min(allRanges.length-1, exactIdx - bt.biasShift));
        recBucket = allRanges[shiftedIdx];
        const dir = bt.biasShift > 0 ? 'down' : 'up';
        shiftNote = `Bias adjusted ${dir} ${Math.abs(bt.biasShift)} bucket (${bt.biasLabel})`;
      }
