#!/usr/bin/env node
/* ============================================================
   PM Scanner — GitHub Actions scan script
...
function isInTriggerWindow(city, nowUtcHour) {
  ...
  /* Trigger if current UTC hour matches (hourly cron tolerance) */
  return Math.round(nowUtcHour) === Math.round(targetUtcHour);
}
...
async function getNwsForecast(city) {
  ...
  for (const p of (data.properties?.periods || [])) {
    const ts = p.startTime?.slice(0, 10);
    let t = p.temperature;
...
/* ============================================================
   POLYMARKET MARKET FETCH + BUCKET PARSING
   ============================================================ */
...
async function main() {
  ...
  const yyyy = today.getFullYear(), month = MONTHS_EN[today.getMonth()], day = today.getDate();
  ...
  for (const city of dueCities) {
    ...
    try {
      ...
      /* Only alert on genuine arbs or strong +EV (20%+) to keep signal quality high
         for an unattended automated alert */
      ...
      const rangeLabel = `${sig.core3[0].rng.lo}${unit}–${sig.core3[2].rng.hi}${unit}`;
      const wuLink = buildWundergroundLink(city, today);

      const msg = [
        `${sig.isArb ? '⚡ GUARANTEED ARB' : '🟢 +EV SIGNAL'} — ${city.name}`,
        `${ds} · forecast ${tempF}°F (${forecastVal}${unit})`,
        `Range: ${rangeLabel} · Cost: ${totalCents}¢`,
        sig.isArb ? `Guaranteed profit: +${100 - totalCents}¢/share` : `EV: +${sig.ev}%`,
        `Bet size: $${BET_SIZE} flat, split across 3 buckets`,
        wuLink ? wuLink : '',
      ...
      newAlerts.push(msg);
      alertedSet.add(key);
      console.log(`  ✅ Signal found: ${sig.isArb ? 'ARB' : '+EV'} ${sig.ev}% — queued for alert.`);
