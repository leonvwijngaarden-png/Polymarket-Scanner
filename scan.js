...
/* ── Live wind, cloud and precipitation from Open-Meteo ── */
async function getLiveWindCloud(city) {
  const url = `https://api.open-meteo.com/v1/forecast` +
    `?latitude=${city.lat}&longitude=${city.lon}` +
    `&current=windspeed_10m,winddirection_10m,cloudcover,precipitation` +
  ...
  /* Noon precipitation probability from hourly */
  const precipProb = data.hourly?.precipitation_probability?.[12] ?? 0;
  ...
  return {
    ...
    precipNow:  Math.round(precipNow * 10) / 10,
...
function getWindCloudAdjustment(cityName, wind, bt) {
  ...
  /* Precipitation bias — high precip probability = runs cooler */
  const precipBucket = wind.precipProb < 20 ? 'low'
    : wind.precipProb < 50 ? 'medium' : 'high';
  const precipBias = wc.precipProbability?.[precipBucket]?.avgBias ?? 0;

  /* Combine: wind direction 50%, cloud 30%, precip 20% */
  const combined = (dirBias * 0.5) + (cloudBias * 0.3) + (precipBias * 0.2);
  const extraShift = Math.round(combined);

  if (Math.abs(combined) < 0.4) return { extraShift: 0, note: null };

  const parts = [];
  if (Math.abs(dirBias) >= 0.4)
    parts.push(`${wind.windCompass} wind → ${dirBias > 0 ? 'warmer' : 'cooler'} (${dirBias > 0 ? '+' : ''}${dirBias.toFixed(1)} buckets)`);
  if (Math.abs(cloudBias) >= 0.4)
    parts.push(`${cloudBucket} skies → ${cloudBias > 0 ? 'warmer' : 'cooler'} (${cloudBias > 0 ? '+' : ''}${cloudBias.toFixed(1)} buckets)`);
  if (Math.abs(precipBias) >= 0.4)
    parts.push(`${precipBucket} rain chance → ${precipBias > 0 ? 'warmer' : 'cooler'} (${precipBias > 0 ? '+' : ''}${precipBias.toFixed(1)} buckets)`);

  return { extraShift, note: parts.filter(Boolean).join(' · ') || null };
}
...
async function main() {
  ...
  for (const city of dueCities) {
    ...
    try {
      ...
      const windLine = wind
        ? `🌬 ${wind.windCompass} ${wind.windSpeedMph}mph · ☁️ ${wind.cloudCover}%${wind.precipProb >= 20 ? ` · 🌧 ${wind.precipProb}% rain` : ''}`
        : null;
