#!/usr/bin/env node
/* ============================================================
   PM Scanner — Backtest v4
   Adds bias direction tracking (warm/cold) per city so scan.js
   can shift bracket recommendations accordingly.
...
async function main() {
  console.log(`PM Scanner Backtest v4 — ${DAYS_BACK} days | ${CITIES.length} cities\n`);
  ...
  for (const city of CITIES) {
    ...
    for (let daysAgo = DAYS_BACK; daysAgo >= 7; daysAgo--) {
      ...
      /* Bias direction: positive = forecast runs warm (predicted higher than actual),
         negative = forecast runs cold (predicted lower than actual).
         Measured in buckets. */
      ...
      /* positive biasDir = forecast bucket index > winner bucket index = forecast ran warm */
      ...
      cityRows.push({ date:dateStr, forecastC:forecast.tempC, forecastF:forecast.tempF,
        winner:market.winnerQuestion, exact, w1, bucketsOff, biasDir });
    }
    ...
    /* Net bias: average of biasDir across all days.
       Positive = forecast consistently runs warm (should shift recommendation DOWN).
       Negative = forecast consistently runs cold (should shift recommendation UP). */
    ...
    const biasShift = Math.round(netBias); /* whole buckets to shift */
    ...
    console.log(`${n.toString().padStart(2)} days | exact ${eh}/${n} (${n?Math.round(eh/n*100):'-'}%) | ±1 ${w1h}/${n} (${n?Math.round(w1h/n*100):'-'}%) | bias: ${biasLabel}`);
    ...
    results.push({
      ...
      biasShift,   /* -1 = shift rec up 1, +1 = shift rec down 1, 0 = no shift */
      biasLabel,
      detail: cityRows,
    });
  }
  ...
  const ranked = results.filter(r=>r.days>=5).sort((a,b)=>b.exactPct-a.exactPct);
  console.log(`\nBest cities (exact hit rate):`);
  ranked.slice(0,5).forEach(r=>console.log(`  ${r.city.padEnd(16)} exact: ${r.exactPct}% | ±1: ${r.within1Pct}% | ${r.biasLabel}`));
  console.log(`Worst cities (exact hit rate):`);
  ranked.slice(-5).reverse().forEach(r=>console.log(`  ${r.city.padEnd(16)} exact: ${r.exactPct}% | ±1: ${r.within1Pct}% | ${r.biasLabel}`));
