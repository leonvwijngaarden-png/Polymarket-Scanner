...
/* ── Wind and cloud conditions at local noon on a given date ──
   Uses ERA5 archive for historical hourly data.
   windDir: degrees (0=N, 90=E, 180=S, 270=W)
   windSpeed: km/h
   cloudCover: 0-100% */
async function getWindCloud(city, dateStr) {
  const url = `https://archive-api.open-meteo.com/v1/archive` +
    `?latitude=${city.lat}&longitude=${city.lon}` +
    `&hourly=windspeed_10m,winddirection_10m,cloudcover` +
  ...
  /* Get values at local noon (hour 12) */
  ...
  return {
    windSpeed: Math.round(windSpeed * 10) / 10,  /* km/h */
...
function computeWindCloudCorrelations(rows) {
  ...
  /* Cloud cover buckets: clear (<25%), partly (25-75%), overcast (>75%) */
  ...
  for (const city of CITIES) {
    ...
    for (let daysAgo = DAYS_BACK; daysAgo >= 7; daysAgo--) {
      ...
      cityRows.push({
        ...
        /* Wind/cloud conditions at local noon */
