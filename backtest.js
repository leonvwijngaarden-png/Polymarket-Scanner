...
/* ── Fetch resolved Polymarket market ──
   The Gamma API's `resolved` flag is unreliable — markets often remain
   resolved:false even weeks after resolution. The correct detection method
   is checking outcomePrices: a resolved YES market has prices[0] near 1.0,
   a resolved NO market has prices[0] near 0.0. */
async function getResolvedMarket(city, dateStr) {
  ...
  for (const m of markets) {
    try {
      ...
      /* YES resolved: price at or very near 1.0 */
      if (yesP >= 0.95) { winner = m; break; }
    } catch {}
  }
